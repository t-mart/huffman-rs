...
fn roundtrip_path<P: AsRef<Path>>(path: P) -> Result<()> {
    ...
    let encoded = table.compress(words.into_iter())?;
    println!("Compress Time: {:?}", start_encode.elapsed());
    let encoded_bytes = encoded.len() / 8;
    println!("Compressd Size: {encoded_bytes} B");
    ...
    let decoded_symbols = table.decompress(&encoded)?;
    let decoded: String = decoded_symbols.into_iter().collect();
    println!("Decompress Time: {:?}", start_decode.elapsed());

    // check equality
    let equal = text.eq(&decoded);
    println!("Decompressed matches original?: {equal}");
